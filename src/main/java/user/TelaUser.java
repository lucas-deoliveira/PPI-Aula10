...
public class TelaUser {
        ...
        window.addWindowListener(new WindowAdapter() {
            public void windowClosing(WindowEvent e) {
                if (socket != null && socket.isConnected()) {
                    try {
                        new UserEnvia(socket, getnomeUsuario(), "3", "");
                ...
                if (socket != null && socket.isConnected()) {
                    ...
                    if ("".equals(name) || "".equals(portaClinet)) {
                        ...
                        try {
                            ...
                            sair.setText("sair");
                            new UserEnvia(socket, getnomeUsuario(), "2", "");
                            new Thread(new UserRecebe(socket)).start();
                        } catch (Exception e2) {
    ...
    public void enviarMsg() {
        ...
        if ("".equals(messages)) {
            ...
            try {
                new UserEnvia(socket, getnomeUsuario() + ": " + messages, "1", "");
                message.setText(null);
            } catch (IOException e1) {
    ...
    public void enviarMsgmsgPriv() {
        ...
        if ("".equals(messages)) {
            ...
            try {
                new UserEnvia(socket, getnomeUsuario() + ": " + messages, "4", getmsgPrivada());
                TelaUser.textMessage.append(getnomeUsuario() + ": " + messages + "\r\n");
                message.setText(null);
            } catch (IOException e1) {
